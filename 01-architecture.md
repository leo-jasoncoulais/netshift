...
- **Partiel** : seul le trafic à destination du réseau `172.30.0.0/16` transite par le tunnel.
...
graph TB
    ...
    subgraph VPS["VPS Hetzner — Netguard (172.30.0.1)"]
        WG0["wg0 — hub WireGuard"]
        CADDY["Caddy — reverse proxy"]
    end

    subgraph Domicile["Domicile — même réseau local"]
        DEBBIE["Debbie (172.30.0.2)<br/>Serveur principal Docker"]
        WINDAUBE["Windaube (172.30.0.3 / .30.10.x)<br/>PC fixe gaming"]
    end

    subgraph Mobile["Machines mobiles"]
        LUNALEX["Lunalex (172.30.0.4 / .30.10.x)<br/>Téléphone"]
        ARTEMIS["Artemis (172.30.0.5 / .30.10.x)<br/>Laptop Arch Linux"]
    end

    subgraph CI["CI/CD"]
        GHA["GitHub Actions (172.30.0.254)<br/>Déploiement SSH"]
    end
...
- **Réseau `172.30.0.0/16` = cercle de confiance total.** Toute machine qui y est connectée (peu importe le sous-réseau, partiel ou full) est considérée de confiance et peut atteindre tous les services exposés en interne. Il n'existe volontairement aucune microsegmentation entre peers à ce stade.
...
- **Roadmap de segmentation** : en cas d'ouverture future du réseau à des tiers (`172.40.0.0/24` prévu), des règles `FORWARD` explicites isoleront ce sous-réseau du cercle de confiance `172.30.0.0/16`.
